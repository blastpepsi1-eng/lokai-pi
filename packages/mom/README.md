...
```bash
npm install @blastpepsi1-eng/lokai-mom
```
...
- run interactive coding agent session: `npx @blastpepsi1-eng/lokai-coding-agent`
- enter `/login` command
...
Update mom anytime with `npm install -g @blastpepsi1-eng/lokai-mom`. This only updates the Node.js app on your host. Anything mom installed inside the Docker container remains unchanged.
