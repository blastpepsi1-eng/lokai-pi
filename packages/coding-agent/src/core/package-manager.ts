...
export class DefaultPackageManager implements PackageManager {
	...
	addSourceToSettings(source: string, options?: { local?: boolean }): boolean {
		...
		if (parsed.type !== "local") {
			throw new Error(`Only local package paths are supported: ${source}`);
		}
	...
	async install(source: string, options?: { local?: boolean }): Promise<void> {
		const parsed = this.parseSource(source);
		const scope: SourceScope = options?.local ? "project" : "user";
		await this.withProgress("install", source, `Adding ${source}...`, async () => {
			if (parsed.type === "local") {
				const baseDir = this.getBaseDirForScope(scope);
				const resolved = this.resolvePathFromBase(parsed.path, baseDir);
	...
	async update(source?: string): Promise<void> {
		if (source) {
			throw new Error(`Package updates are disabled for local-only packages: ${source}`);
		}
	}
		...
		for (const { pkg, scope } of sources) {
			...
			if (parsed.type === "npm") {
				if (onMissing) {
					...
					if (action === "error") throw new Error(`Remote package sources are disabled: ${sourceStr}`);
				}
				continue;
			}
	 ...
	 * Remote package identities are retained only so existing disabled entries dedupe consistently.
	 */
