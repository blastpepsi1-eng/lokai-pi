...
| Package | Purpose |
|---------|---------|
| `@blastpepsi1-eng/lokai-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@blastpepsi1-eng/lokai-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@blastpepsi1-eng/lokai-tui` | TUI components for custom rendering |

npm dependencies work too for local development. Add a `package.json` next to your extension (or in a parent directory), run `npm install`, and imports from `node_modules/` are resolved automatically.

For local prebuilt pi packages referenced with `pi install`, runtime deps must already exist in the package directory or be bundled with the package.
...
```typescript
import { isToolCallEventType } from "@blastpepsi1-eng/lokai-coding-agent";
...
```typescript
import { isBashToolResult } from "@blastpepsi1-eng/lokai-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@blastpepsi1-eng/lokai-coding-agent";
...
```typescript
import { SessionManager } from "@blastpepsi1-eng/lokai-coding-agent";
...
```typescript
import type { AutocompleteItem } from "@blastpepsi1-eng/lokai-tui";
...
```typescript
import { withFileMutationQueue } from "@blastpepsi1-eng/lokai-coding-agent";
...
**Important:** Use `StringEnum` from `@blastpepsi1-eng/lokai-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@blastpepsi1-eng/lokai-coding-agent";
...
```typescript
import { createBashTool } from "@blastpepsi1-eng/lokai-coding-agent";
...
```typescript
import { keyHint } from "@blastpepsi1-eng/lokai-coding-agent";
...
```typescript
import { Text, Component } from "@blastpepsi1-eng/lokai-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@blastpepsi1-eng/lokai-coding-agent";
import { matchesKey } from "@blastpepsi1-eng/lokai-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@blastpepsi1-eng/lokai-coding-agent";
